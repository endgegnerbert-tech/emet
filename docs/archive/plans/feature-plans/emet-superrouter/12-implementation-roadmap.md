...
Run the Phase 12 roadmap audit to verify that every slice has concrete evidence artifacts, rollback hooks, and safety reports before continuing rollout:

```bash
node scripts/router/audit-implementation-roadmap.mjs
```

The audit writes `metrics/router/implementation-roadmap.json` and fails closed when a slice is missing required implementation evidence.
