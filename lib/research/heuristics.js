...
export function classifySourceType(url, title = "") {
  ...
  if (/api\.github\.com\/repos\/[^/]+\/[^/]+\/(?:releases|tags)/.test(lower)) return "github_repo";
  if (/github\.com\/[^/]+\/[^/]+/.test(lower)) return "github_repo";
  if (/npmjs\.com\/package\/|registry\.npmjs\.org\/|pypi\.org\/(?:project|pypi)\/|crates\.io\/(?:crates|api\/v1\/crates)\/|docs\.rs\/|search\.maven\.org\/|central\.sonatype\.com\//.test(lower)) return "official_doc";
  ...
  if (/modelcontextprotocol\.io\/specification\/|developer\.mozilla\.org\/|(?:html|dom|fetch)\.spec\.whatwg\.org|w3\.org\/|nodejs\.org\/(?:api|en\/blog\/release)|docs\.python\.org\/|peps\.python\.org\/|react\.dev\/|kubernetes\.io\/docs\/|docs\.docker\.com\/|docs\.github\.com\//.test(lower)) return "official_doc";
...
export function isAuthoritativeUrl(url) {
  ...
  if (/github\.com\/[^/]+\/[^/]+(#readme|\/tree\/[^/]+\/docs|\/blob\/|\/releases|\/tags)/.test(lower)) return true;
  if (/api\.github\.com\/repos\/[^/]+\/[^/]+\/(?:releases|tags)/.test(lower)) return true;
  if (/github\.com\/[^/]+\/[^/]+$/.test(lower)) return true;
  if (/npmjs\.com\/package\/|registry\.npmjs\.org\/|pypi\.org\/(?:project|pypi)\/|crates\.io\/(?:crates|api\/v1\/crates)\/|docs\.rs\/|search\.maven\.org\/|central\.sonatype\.com\/|modelcontextprotocol\.io\/specification\/|developer\.mozilla\.org\/|(?:html|dom|fetch)\.spec\.whatwg\.org|w3\.org\/|nodejs\.org\/(?:api|en\/blog\/release)|docs\.python\.org\/|peps\.python\.org\/|react\.dev\/|kubernetes\.io\/docs\/|docs\.docker\.com\/|docs\.github\.com\/|arxiv\.org|pubmed\.ncbi\.nlm\.nih\.gov|semanticscholar\.org|doi\.org|research\.ibm\.com|research\.google/.test(lower)) return true;
