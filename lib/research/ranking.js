...
export function scoreSearchResult(result, query, config = {}) {
  ...
  if (result.directOfficialTarget) score += 12 + (Number(result.officialTargetPriority) || 0);
...
export function scoreSourceEntry(source, query = "") {
  ...
  const explicitNonAuthoritative = source?.authoritative === false;
  const githubStatePage = /github\.com\/[^/]+\/[^/]+\/(issues|pull|pulls|discussions)\//i.test(url);
  if (explicitNonAuthoritative || githubStatePage) authoritative = false;
