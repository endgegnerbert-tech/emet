...
export const WEB_FETCH_DEFAULT_TEXT_LIMIT = 200_000;
...
export async function webFetch(url, signal, config = {}) {
  const requestedLimit = Number(config.maxBytes ?? config.pageTextLimit ?? config.maxTextLength ?? WEB_FETCH_DEFAULT_TEXT_LIMIT);
  const pageTextLimit = Number.isFinite(requestedLimit) && requestedLimit > 0
    ? Math.min(requestedLimit, WEB_FETCH_DEFAULT_TEXT_LIMIT)
    : WEB_FETCH_DEFAULT_TEXT_LIMIT;
  ...
  if (!page) {
    const reason = fetchDiagnostics[0]?.reason || (shouldSkipUrl(url) ? "blocked_url" : "unreachable");
    await logResearchEvent("web_fetch_fail", { url, reason });
    return { ok: false, url, error: `Could not fetch ${url}: ${reason}`, diagnostics: fetchDiagnostics };
  }
  const fullText = page.fullText || page.text || "";
  const text = fullText.length > pageTextLimit ? fullText.slice(0, pageTextLimit) : fullText;
  const truncated = fullText.length > text.length;
  await logResearchEvent("web_fetch_ok", { url, title: page.title, textLength: text.length, originalLength: fullText.length, truncated });
  return {
    ok: true,
    url: page.url || url,
    title: page.title || "",
    ...
    textLimit: pageTextLimit,
    nextOffset: truncated ? text.length : null,
    contentType: page.contentType || "text/html",
    codeBlocks: Array.isArray(page.codeBlocks) ? page.codeBlocks : extractCodeBlocks(text),
