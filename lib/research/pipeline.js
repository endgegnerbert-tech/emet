...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  if (flowPolicy.runMode === "checkpoint" && hasCommunityRetrieval && (!config.action || config.action === "search" || config.action === "refine")) {
  ...
  for (let turn = 0; turn < Math.max(1, activeConfig.maxTurns || 1); turn++) {
    ...
    const [searchGroups, communitySearch] = await Promise.all([
      Promise.all(queriesThisTurn.map((subquery) => searchDuckDuckGo(subquery, signal, activeConfig))),
    ...
    const pageCandidates = fetchedCandidates.map((page) => {
      ...
      if (!source?.community) return page;
      return {
        ...page,
