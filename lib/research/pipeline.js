...
import { applyGuardrailsToResearchConfig, buildResearchGuardrails, guardrailVetoesDomainDowngrade, resolveGuardrailedMinSources, snapshotGuardrails } from "../research-guardrails.js";
import { shouldRequireAuthoritativeSources } from "../research-output.js";
import { planResearch } from "../planner.js";
import { clearResearchMemory, getResearchMemory, hashResearchQuery, modeCacheKey, topicCacheKey, canUseTopicCache, readCachedResult, setResearchMemory, writeCachedResult, writeDevCacheResult, readPageSnapshot, writePageSnapshot } from "../research-memory.js";
...
// ponytail: ML removed — heuristic-only domain routing
export async function resolveQuestionDomain(query, mode, signal, guardrails = null, _preflightPrediction = null) {
  ...
  const selectedDomain = selection.shouldBypassLearnedRouter ? selection.primaryDomain : fallback;
  const guardrailVeto = guardrailVetoesDomainDowngrade(heuristicDomain, selectedDomain, guardrails);
  const finalDomain = guardrailVeto ? heuristicDomain : selectedDomain;

  return {
    heuristicDomain,
    predictedDomain: null,
    finalDomain,
    decisionSource: guardrailVeto ? "guardrail" : (selection.shouldBypassLearnedRouter ? selection.decisionSource : "heuristic"),
    decisionReason: selection.shouldBypassLearnedRouter
      ? (guardrailVeto ? "guardrail_veto_domain_downgrade" : (selection.decisionSource === "forced" ? "forced_domain" : "explicit_domain"))
      : "heuristic_fallback",
    latencyMs: null,
  };
}
...
async function runSelectedUrlFetch(query, ctx, signal, config, guardrailSnapshot, flowPolicy) {
  const urls = [...new Set((Array.isArray(config.selectedUrls) ? config.selectedUrls : []).filter(Boolean))];
  const pages = [];
  const fetchDiagnostics = [];
  const fetchConfig = { ...config, isolate: true, fetchDiagnostics };
  for (const url of urls.slice(0, Math.min(config.maxPages || config.maxSites || 5, 10))) {
    try {
      const page = await fetchPageSource(url, signal, fetchConfig);
      if (page?.text) pages.push(page);
    } catch {
      // fetchDiagnostics carries the policy/network reason where available.
    }
  }

  const synthesis = pages.length
    ? await synthesizeResearch(query || "selected URLs", pages, ctx, signal)
    : { answer: "No selected URLs could be fetched.", bullets: [], citations: [], sources: [] };
  const sources = pages.map((page) => ({
    title: page.title || page.url,
    url: page.url,
    sourceType: page.sourceType || classifySourceType(page.url, page.title || ""),
    authoritative: Boolean(page.authoritative),
    freshness: normalizeResearchFreshness(page.freshness, page.publishDate),
  }));
  const authoritativeSourcesFound = sources.some((source) => source.authoritative || source.sourceType === "official_doc");
  const sufficient = pages.length > 0 && (!shouldRequireAuthoritativeSources(config) || authoritativeSourcesFound);
  const confidence = buildConfidenceSummary(pages, { conflictDetected: false, followupRounds: 0 });
  const contentText = formatResultText({
  ...
  return {
    ...
    schemaVersion: 1,
    outcome: sufficient ? "sufficient" : "partial_success",
    reason: pages.length ? "selected_urls_fetched" : "selected_urls_unreadable",
    query,
    mode: config.mode,
    ...
    sources,
    sourceTypes: [...new Set(sources.map((source) => source.sourceType).filter(Boolean))],
    guardrail_flags: guardrailSnapshot.guardrail_flags,
    format: config.format,
    confidence,
    confidenceScore: sufficient ? 0.8 : 0.3,
    authoritativeSourcesFound,
    sufficient,
    followupRecommended: !sufficient,
    missingAspects: sufficient ? [] : [shouldRequireAuthoritativeSources(config) && !authoritativeSourcesFound ? "authoritative sources" : "readable selected URLs"].filter(Boolean),
    openSubQuestions: [],
    unverifiedClaims: [],
    fetchDiagnostics,
    contentText,
...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  const preflightPrediction = null;
  ...
  const queryUnderstandingPrediction = null;
  ...
  const isCheckpointContinuation = Boolean(config.sessionId && ["search", "refine", "fetch", "synthesize"].includes(config.action));
  ...
  if ((flowPolicy.runMode === "checkpoint" && hasCommunityRetrieval && (!config.action || config.sessionId || config.action === "search" || config.action === "refine")) || isCheckpointContinuation) {
    return runCommunityCheckpoint(query, ctx, signal, onUpdate, config, flowPolicy);
  }

  if (Array.isArray(config.selectedUrls) && config.selectedUrls.length > 0 && !config.sessionId) {
    return runSelectedUrlFetch(query, ctx, signal, config, guardrailSnapshot, flowPolicy);
  }
  ...
  if (!config.isolate && !config.force && !config.interactive && !hasCommunityRetrieval) {
    ...
    let persistentHit = config.rawPages ? null : readCachedResult(cacheKey);
    if (!persistentHit && canUseTopicCache(query, config)) {
  ...
  for (let turn = 0; turn < Math.max(1, activeConfig.maxTurns || 1); turn++) {
    ...
    const fetchedCandidates = activeConfig.mode === "fast"
      ...
      : await mapWithConcurrency(results, activeConfig.concurrentQueries || 3, (result) => fetchPageSource(result.url, signal, {
          ...activeConfig,
          query,
          minPageText: result.directOfficialTarget ? 1 : activeConfig.minPageText,
        }));
    ...
    // ponytail: ML removed — heuristic-only conflict detection
    turnTrace.conflict = {
      heuristicDetected: conflictDetected,
    ...
    // ponytail: ML removed — heuristic-only sufficiency
    ...
    // ponytail: ML removed — heuristic-only followup
  ...
  if (!config.rawPages) writeCachedResult(cacheKey, slimResult, config.cacheTtlMs);
  ...
  const topicKey = topicCacheKey(query, config);
  if (topicKey !== cacheKey && canUseTopicCache(query, config)) {
