...
export async function resolveQuestionDomain(query, mode, signal, guardrails = null, _preflightPrediction = null) {
  ...
  return {
    ...
    finalDomain: selection.shouldBypassLearnedRouter ? selection.primaryDomain : fallback,
    decisionSource: selection.shouldBypassLearnedRouter ? selection.decisionSource : "heuristic",
    decisionReason: selection.shouldBypassLearnedRouter
      ? (selection.decisionSource === "forced" ? "forced_domain" : "explicit_domain")
      : "heuristic_fallback",
    latencyMs: null,
  };
}
...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  for (let turn = 0; turn < Math.max(1, activeConfig.maxTurns || 1); turn++) {
    ...
    // ponytail: ML removed — heuristic-only followup, legacy action always null
