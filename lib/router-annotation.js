export const ANNOTATION_LABELS = {
  conflict: ["no_conflict", "needs_review", "resolved_by_authority", "resolved_by_recency", "resolved_by_version", "open_conflict"],
  sufficiency: ["sufficient", "need_more_sources", "need_authority", "need_primary_source", "need_recency", "need_version_context", "need_conflict_resolution"],
  followup: ["stop", "need_more_sources", "need_authority", "need_primary_source", "need_recency", "need_version_context", "need_conflict_resolution", "ask_clarifying_question"],
};
...
function isPrimarySourceQuery(query = "") {
  return /\b(paper|papers|study|studies|arxiv|doi|pubmed|clinical guideline|rfc|standard|specification|primary source|official advisory|vendor advisory)\b/i.test(query);
}
...
export function suggestAnnotation(task, row = {}) {
  ...
  if (task === "conflict") {
    ...
    if (versioned && authoritative) {
      return { label: "resolved_by_version", rationale: "The disagreement appears version-specific, so exact version evidence should resolve it." };
    }
    ...
    return candidateLabel === "conflict"
      ? { label: "open_conflict", rationale: "The snippets suggest disagreement, and there is no clear authoritative, versioned, or fresher winner." }
      : { label: "needs_review", rationale: "The disagreement signal is ambiguous and should be reviewed before training." };
  }

  if (candidateLabel === "conflict" || meta.conflictSummary) {
    return { label: "need_conflict_resolution", rationale: "The evidence state indicates unresolved disagreement before this can be answered safely." };
  }
  if (isPrimarySourceQuery(query) && (counts.paper + counts.official_doc === 0 || !authoritative)) {
    return { label: "need_primary_source", rationale: "This query needs a primary paper, standard, advisory, or publisher source before answering." };
  }
  if (!authoritative) {
