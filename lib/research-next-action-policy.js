...
import { extractQueryAspectFlags } from "./research/heuristics.js";
...
export function followupGapFromPolicyAction(action = "") {
  return {
...
export function buildPolicyFollowUpQuery(query = "", policyDecision = {}, options = {}) {
  const gapAction = followupGapFromPolicyAction(policyDecision.action);
  if (gapAction === "stop") return null;
  return buildActionBasedFollowUpQuery(query, gapAction, options);
}
