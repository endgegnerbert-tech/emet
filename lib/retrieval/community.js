...
const DEFAULT_COMMUNITY_PLATFORMS = ["hn", "github", "rss"];

export function selectedCommunityPlatforms(query, config = {}, flowPolicy = {}) {
  if (Array.isArray(config.platforms) && config.platforms.length > 0) return config.platforms;
...
function communityNextActions(session, sources, maxTurns, currentQuery) {
  const actions = [];
  if (sources.length) {
...
export async function runCommunityCheckpoint(query, ctx, signal, onUpdate, config, flowPolicy) {
  ...
  if (action === "fetch" || action === "synthesize") {
    return {
      ok: false,
      schemaVersion: 1,
      action,
      ...
      query,
      currentQuery: config.queryOverride || query,
      sufficient: false,
      followupRecommended: true,
      missingAspects: ["pipeline synthesis"],
      nextActions: [{ action: "synthesize", reason: "Run the normal pipeline without checkpointing for synthesis", options: { interactive: false } }],
      contentText: "Community fetch/synthesis is handled by the normal research pipeline; rerun without interactive checkpointing.",
    };
  }

  const session = getOrCreateSession(config.sessionId, query, config._checkpointMaxTurns || COLLECTOR_MAX_TURNS_DEFAULT);
  ...
  if (session.turn > maxTurns) {
    return {
      ...
      action: "search",
      retrievalClass: "community",
      legacyAction: "collector_interactive",
  ...
  const platforms = selectedCommunityPlatforms(query, config, flowPolicy);
  ...
  const nextActions = communityNextActions(session, community.results, maxTurns, currentQuery);
...
async function handleCollectorEntry(query, _ctx, _signal, _onUpdate, config) {
  ...
  if (session.turn > maxTurns) {
    return {
      ok: false, action: "search", legacyAction: "collector_interactive", retrievalClass: "community", schemaVersion: 1, sessionId: session.id,
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      const withIds = (result.results || []).map((r, i) => {
        const normalized = normalizeCollectorResult(platform, r, i);
        return {
          id: normalized.id,
          title: r.title,
          url: r.url,
  ...
  return {
    ok: true, action: "search", legacyAction: "collector_search", retrievalClass: "community", schemaVersion: 1, sessionId: session.id,
    query: session.query, currentQuery, turn: session.turn,
...
async function handleCollectorFetch(session, config, currentQuery) {
  ...
  return {
    ok: true, action: "fetch", legacyAction: "collector_fetch", retrievalClass: "community", schemaVersion: 1, sessionId: session.id,
    query: session.query, currentQuery, turn: session.turn,
...
async function handleCollectorSynthesize(session, config, currentQuery, ctx, signal) {
  ...
  if (session.fetchedPages.length === 0) {
    return {
      ok: false, action: "synthesize", legacyAction: "collector_synthesize", retrievalClass: "community", schemaVersion: 1, sessionId: session.id,
      query: session.query, currentQuery, turn: session.turn,
  ...
  const result = {
    ok: true, action: "synthesize", legacyAction: "collector_synthesize", retrievalClass: "community", schemaVersion: 1, sessionId: session.id,
    query: session.query, currentQuery, turn: session.turn,
