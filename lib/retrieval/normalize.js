...
// Pure module: no network, no collector calls.
...
const PLATFORM_PROFILES = {
  hn: { evidenceRole: "community_signal", stability: "stable", auth: "none" },
  v2ex: { evidenceRole: "community_signal", stability: "stable", auth: "none" },
  github: { evidenceRole: "mixed_signal", stability: "stable", auth: "none" },
  rss: { evidenceRole: "publisher_signal", stability: "stable", auth: "none" },
  youtube: { evidenceRole: "media_signal", stability: "best_effort", auth: "none" },
  reddit: { evidenceRole: "community_signal", stability: "best_effort", auth: "none" },
  bilibili: { evidenceRole: "media_signal", stability: "best_effort", auth: "none" },
  weibo: { evidenceRole: "community_signal", stability: "fragile", auth: "none" },
};
...
function profileForPlatform(platform) {
  return PLATFORM_PROFILES[platform] || { evidenceRole: "community_signal", stability: "best_effort", auth: "none" };
}
...
function generateId(platform, item, index) {
  ...
  if (nativeId && !String(nativeId).startsWith(`${platform}:`)) return `${platform}:${nativeId}`;
  if (nativeId) return String(nativeId);
  const stableKey = item.url || item.title || item.snippet || JSON.stringify(item) || String(index);
  return `${platform}:${shortHash(stableKey)}`;
}
...
function baseSignals(platform, item, kind) {
  const raw = item.signals || {};
  const profile = profileForPlatform(platform);
  return {
    platform,
    kind,
    evidenceRole: profile.evidenceRole,
    platformStability: profile.stability,
    auth: profile.auth,
    createdAt: item.createdAt || item.created_at || raw.createdAt || raw.updatedAt || null,
  };
}
...
function extractSignals(platform, item) {
  ...
  switch (platform) {
    case "hn":
      return {
        ...baseSignals(platform, item, "story"),
        author: item.author || item.by || "",
        comments: item.num_comments || item.comments || raw.comments || 0,
        points: item.points || item.score || 0,
        objectID: item.objectID || raw.objectID || null,
      };
    case "v2ex":
      return {
        ...baseSignals(platform, item, "topic"),
        author: item.author || item.member?.username || "",
        comments: item.replies || raw.comments || 0,
        points: 0,
      };
    case "github":
      return {
        ...baseSignals(platform, item, item.kind || "repo"),
        author: item.author || item.owner?.login || "",
        comments: item.open_issues_count || 0,
        points: item.stargazers_count || item.stars || item.score || 0,
        language: raw.language || item.language || null,
      };
    case "rss":
      return {
        ...baseSignals(platform, item, "article"),
    ...
    case "youtube":
      return {
        ...baseSignals(platform, item, "video"),
        author: item.author || item.channelTitle || item.channel || "",
        comments: item.commentCount || raw.comments || 0,
        points: item.viewCount || item.views || item.score || 0,
      };
    default:
      return {
        ...baseSignals(platform, item, "unknown"),
        author: item.author || "",
