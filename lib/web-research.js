    ...
    if (isPdfUrl(url, contentType) || contentType.includes("pdf")) {
      ...
      if (pdfResult && pdfResult.text) {
        ...
        const stored = final && withinTimeframe(final, config)
          ...
          : null;
        await logResearchEvent("fetch_end", { url, via: "pdf_extraction", success: Boolean(stored), page: stored ? { title: stored.title, sourceType: stored.sourceType, textLength: stored.text?.length || 0 } : null });
        return stored;
      }
    }
