...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  // Collector interactive branch — early return before full web pipeline
  if (shouldRunCollectorInteractive(query, modeOptions)) {
    return runCollectorInteractive(query, ctx, signal, onUpdate, modeOptions);
  }
...
// --- Collector interactive mode ---
// ponytail: in-memory session map, no DB, no framework
const COLLECTOR_SESSION_TTL = 30 * 60 * 1000;
const COLLECTOR_MAX_SESSIONS = 100;
const COLLECTOR_MAX_TURNS_DEFAULT = 3;

/** In-memory sessions keyed by randomUUID. Exported for testing. */
const collectorSessions = new Map();

function getOrCreateSession(sessionId, query, maxTurns) {
  const now = Date.now();
  for (const [id, s] of collectorSessions) {
    if (now - s.createdAt > COLLECTOR_SESSION_TTL) collectorSessions.delete(id);
  }
  ...
  if (collectorSessions.size >= COLLECTOR_MAX_SESSIONS) {
    collectorSessions.delete(collectorSessions.keys().next().value);
  }
  const id = sessionId || randomUUID();
  const session = { id, createdAt: now, query: query || "", turn: 0, maxTurns: maxTurns || COLLECTOR_MAX_TURNS_DEFAULT, collectorResults: [], fetchedPages: [] };
...
function shouldRunCollectorInteractive(query, config) {
...
async function runCollectorInteractive(query, ctx, signal, onUpdate, config) {
  ...
  if (action === "synthesize") return handleCollectorSynthesize(session, config, currentQuery, ctx, signal);
...
export { compactResearchPayload, clearResearchMemory, collectorSessions, shouldRunCollectorInteractive, runCollectorInteractive };
