...
function snapshotSearchResult(result = {}) {
  return {
    title: result.title || "",
    url: result.url || "",
...
function annotateVersionSignals(entry = {}, versionContext) {
...
function snapshotPageForTrace(page = {}) {
...
function buildTraceSourceSummary(query, pages = []) {
  ...
  try {
    ...
    if ((!page && assessment?.weak) || assessment?.dynamic || assessment?.blocked) {
      ...
      if (scrapling?.body) {
        ...
        await logResearchEvent("fetch_scrapling_unavailable", { url, mode: assessment.mode, error: scrapling?.error, runtime: getScraplingRuntimeStatus?.() || null });
      }
    }
...
export async function synthesizeResearch(query, pages, ctx, signal) {
  const versionContext = extractVersionContext(query);
  await logResearchEvent("synthesis_start", { query, versionContext, pages: pages.map((page) => ({ title: page.title, url: page.url, sourceType: page.sourceType, textLength: page.text?.length || 0 })) });
  const prompt = [
...
async function resolveQuestionDomain(query, mode, signal, guardrails = null) {
  ...
  try {
    ...
    const domain = chooseTinyRouterDomain(fallback, tinyDomain, { guardrails });
    ...
    if (tinyDomain && domain === fallback && tinyDomain !== fallback) {
      await logResearchEvent("tiny_router_fallback", { task: "domain", query, mode: normalizedMode, heuristicDomain: fallback, predictedDomain: tinyDomain, reason: "guardrail_not_downgraded", guardrails: snapshotGuardrails(guardrails) });
      return {
        heuristicDomain: fallback,
        predictedDomain: tinyDomain,
        finalDomain: fallback,
        decisionSource: "heuristic",
        decisionReason: "guardrail_not_downgraded",
        latencyMs: tinyLatencyMs,
      };
    }
...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  const domainDecision = await resolveQuestionDomain(query, mode, signal, guardrails);
  const domain = domainDecision.finalDomain;
  const queryUnderstandingPrediction = await classifyQueryUnderstandingWithTinyRouter(query, modeOptions.mode || mode, signal);
  const queryUnderstandingPlanning = resolveQueryUnderstandingPlanning(
    getResearchConfig(typeof mode === "object" ? { ...mode, domain } : { mode, domain }),
    query,
    queryUnderstandingPrediction,
    { domain, mode: modeOptions.mode || mode },
  );
  ...
  for (let turn = 0; turn < Math.max(1, config.maxTurns || 1); turn++) {
    ...
    const minSources = resolveGuardrailedMinSources(config, mergedPages);
    ...
    if (sufficiency.sufficient || turn === (config.maxTurns - 1)) {
      turnTrace.stopReason = sufficiency.sufficient ? "sufficient" : "max_turns_reached";
    ...
    const sourcesMeta = sourceMetaFromSources(mergedPages);
    ...
    if (action === "stop") {
      ...
      turnTrace.stopReason = "router_suggested_stop";
      traceTurns.push(turnTrace);
      break;
    }
  ...
  const followupSourcesMeta = sourceMetaFromSources(mergedPages);
  const result = {
    ok: true,
    action: "web_research",
    query,
