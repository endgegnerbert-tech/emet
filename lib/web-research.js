...
function buildTraceSourceSummary(query, pages = []) {
  const structuredSources = structuredSourcesFromPages(pages);
  ...
  const normalizedFreshness = pages.map((page) => normalizeResearchFreshness(page.freshness, page.publishDate));
  return {
    queryFlags,
    structuredSources,
    distinctDomainCount: distinctDomains.size,
    followupInput: sourceMetaFromSources(pages),
    freshnessCounts: {
    ...
    conflictStructuredFeatures: extractConflictStructuredFeaturesFromPages(query, pages, pages.length > 1 ? "candidate_conflict" : ""),
    sufficiencyStructuredFeatures: extractSufficiencyStructuredFeaturesFromPages(query, pages),
  };
}
