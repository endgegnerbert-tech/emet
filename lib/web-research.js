...
function modeCacheKey(query, config) {
  return `${config.mode}:${hashResearchQuery(query)}:${hashText(JSON.stringify({
    ...
    domainFamily: config.domainFamily || "",
...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  const queryUnderstandingPlanning = resolveQueryUnderstandingPlanning(
    getResearchConfig(typeof mode === "object" ? { ...mode, query, domain } : { mode, query, domain }),
    query,
  ...
  for (let turn = 0; turn < Math.max(1, config.maxTurns || 1); turn++) {
    ...
    const turnEvidenceState = buildTurnEvidenceState({ query, sources: mergedPages, config, turn: turn + 1 });
    ...
    if (sufficiency.sufficient || turn === (config.maxTurns - 1)) {
      ...
      turnTrace.evidenceState = buildTurnEvidenceState({ query, sources: mergedPages, config, turn: turn + 1, turnTrace });
