...
export async function runWebResearch(query, ctx, signal, onUpdate, mode = "fast") {
  ...
  const preflightPrediction = await classifyPreflightWithTinyRouter(query, modeOptions.mode || mode, signal);
  ...
  const domainDecision = await resolveQuestionDomain(query, mode, signal, guardrails, preflightPrediction);
  const domain = domainDecision.finalDomain;
  const queryUnderstandingPrediction = preflightPrediction?.queryUnderstanding || await classifyQueryUnderstandingWithTinyRouter(query, modeOptions.mode || mode, signal);
