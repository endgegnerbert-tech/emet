...
function hasDocsLikeSignals(url = "", title = "") {
  const lower = String(url || "").toLowerCase();
  return /\/docs?\b|documentation|developer|reference|official/.test(lower) || /official|documentation|reference|guide/i.test(title);
}

function isSecondaryDocsHost(hostname = "") {
  return /(^|\.)(deepwiki\.com|sureprompts\.com|mcpservers\.com|cursorcommunity\.com|deepwiki\.org)$/.test(hostname);
}

function isLikelyOfficialDocsHost(hostname = "") {
...
export function classifySourceType(url, title = "") {
  ...
  if (isLikelyOfficialDocsHost(hostname) && hasDocsLikeSignals(url, title)) return "official_doc";
  if ((/\.edu\/|\.ac\.uk\//.test(lower) || /(^|\.)(gov|edu)$/.test(hostname) || /\.ac\.uk$/.test(hostname)) && !isSecondaryDocsHost(hostname)) return "official_doc";
  if (hasDocsLikeSignals(url, title) && !isSecondaryDocsHost(hostname) && !/community|forum|mirror|aggregator/.test(hostname)) return "official_doc";
  return "other";
}
...
export function isAuthoritativeUrl(url) {
  ...
  if (isLikelyOfficialDocsHost(hostname) && hasDocsLikeSignals(url, "")) return true;
...
export function detectCoverageGaps(input = {}) {
  ...
  const authoritativeFromSources = sources.some((source) => source.authoritative || source?.sourceType === "official_doc" || source?.sourceType === "paper");
...
export async function extractPageSnapshot(html, url) {
  ...
  if (article) {
    // Convert HTML content to plain text via basic extraction on the article content
