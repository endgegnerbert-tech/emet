...
export function sourceMetaFromSources(sources = []) {
  return {
    has_authority: sources.some(isAuthoritativeResearchSource),
    has_forum: sources.some((source) => source.sourceType === "forum" || /forum|reddit|stack/i.test(source.url || "")),
    has_news: sources.some((source) => source.sourceType === "news" || isNewsSourceUrl(source.url, source.title)),
    has_recent: sources.some((source) => isRecentResearchFreshness(source.freshness, source.publishDate)),
    has_version_match: sources.some((source) => source?.versionSignals?.exactVersionMatch || source?.versionSignals?.partialVersionMatch),
    has_changelog: sources.some((source) => ["changelog", "release_notes", "breaking_changes"].includes(source?.versionSignals?.pageKind)),
    has_migration: sources.some((source) => source?.versionSignals?.pageKind === "migration_guide"),
